...
const Billing = () => {
  ...
  useEffect(() => {
    if (profileData) {
      ...
      console.log(profileData.locations[0]);
  ...
  const handleSort = (key) => {
    if (key === "updatedAt") {
      const newOrder =
        sortConfig.sortBy === "updatedAt" && sortConfig.order === "desc"
          ? "asc"
          : "desc";
      setSortConfig({ sortBy: "updatedAt", order: newOrder });
    } else if (key === "status") {
      const newOrder =
        sortConfig.sortBy === "status" && sortConfig.order === "asc"
          ? "desc"
          : "asc";
  ...
  return (
    <div className="overflow-y-auto h-full w-full sm:rounded-lg">
      {isLoading ? (
        <Loader />
      ) : (
        <>
