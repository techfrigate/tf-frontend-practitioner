      ...
      <div className="flex space-x-10 gap-1">
        <div className="space-y-1.5">
          ...
          >
            ...
            {["Packages", "Diagnostic", "Service"].map((type) => (
        ...
        <div className="space-y-1.5">
          <label className="block text-sm font-medium text-gray-700">
            {category === "Packages" ? "Package" : "Service"}<span className="text-red-500">*</span>
          </label>
          <select
            className="w-[24rem] p-3 bg-white border-2 border-gray-200 rounded-xl text-gray-900
              focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 
            ...
            disabled={!category}
          >
            <option value="">Select {category === "Packages" ? "Package" : "Service"}</option>
            {services.map((service) => (
              <option 
                key={service._id} 
                value={category === "Packages" ? service.packageName : service.name}
              >
                {category === "Packages" ? service.packageName : service.name}
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="mt-3 flex justify-end">
        ...
        >
          <span className="relative z-10 flex items-center space-x-2">
            ...
            <span>Add {category === "Packages" ? "Package" : "Service"}</span>
          </span>
