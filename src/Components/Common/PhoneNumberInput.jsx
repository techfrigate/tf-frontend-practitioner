...
const PhoneNumberInput = ({
  ...
  useEffect(() => {
    if (dialCode !== localDialCode) {
      setLocalDialCode(dialCode);
    }
    if (value !== localNumber) {
      setLocalNumber(value);
    }
  }, [dialCode, value, localDialCode, localNumber]);

  const handleDialCodeChange = useCallback((e) => {
    const newDialCode = e.target.value;
    setLocalDialCode(newDialCode);
    onChangeDialCode(newDialCode);
    setLocalNumber("");
    onChangeNumber("");
  }, [onChangeDialCode, onChangeNumber]);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col mt-1.5">
        <div
          className={`flex border h-10 rounded-lg transition-all ${
            isInvalid
              ? "border-red-500 shadow-md shadow-red-400/30"
              : "border-gray-300 focus-within:border-[#64C6B0] focus-within:shadow-md focus-within:shadow-[#64C6B0]/30"
          }`}
